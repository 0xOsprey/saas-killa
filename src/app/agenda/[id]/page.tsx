...
import { db } from '@/db';
import { awards, rooms, slots, submissions, tracks, users } from '@/db/schema';
...
export default async function SubmissionDetailPage({
  ...
  const wins = await db
    ...
    .from(awards)
    .where(eq(awards.winnerSubmissionId, row.id));
  ...
  return (
    <article className="mx-auto max-w-2xl space-y-5">
      <PageHeader
        ...
        description={
          row.startsAt && row.roomName
            ? `${dayLabel(row.startsAt, event.timezone)} at ${timeOfDay(row.startsAt, event.timezone)} · ${row.roomName}`
            : 'Not scheduled yet'
        }
      />
      ...
      {row.slidesUrl || row.recordingUrl || row.resourcesNote ? (
        <Card className="space-y-3">
          <h2 className="text-sm font-semibold text-ink">Materials</h2>
          <div className="flex flex-wrap gap-2">
            {row.slidesUrl ? (
              <LinkButton href={row.slidesUrl} variant="secondary">
                Slides
              </LinkButton>
            ) : null}
            {row.recordingUrl ? (
              <LinkButton href={row.recordingUrl} variant="secondary">
                Recording
              </LinkButton>
            ) : null}
          </div>
          {row.resourcesNote ? (
            <p className="whitespace-pre-wrap text-sm text-muted">{row.resourcesNote}</p>
          ) : null}
        </Card>
      ) : null}
    </article>
