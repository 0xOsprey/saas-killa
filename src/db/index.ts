...
const globalForDb = globalThis as unknown as {
  __sessionboardSql?: ReturnType<typeof postgres>;
};

function client() {
  if (!globalForDb.__sessionboardSql) {
    globalForDb.__sessionboardSql = postgres(env().DATABASE_URL, { max: 10 });
  }
  return globalForDb.__sessionboardSql;
}
