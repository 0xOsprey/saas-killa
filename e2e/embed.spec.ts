...
const BOTH_WIDGETS =
  '<div data-saas-killa="speakers"></div><div data-saas-killa="agenda"></div>';
...
test('the widgets say the programme is closed rather than rendering nothing', async ({
  ...
  const gallery = page.locator('[data-saas-killa="speakers"]');
  const itinerary = page.locator('[data-saas-killa="agenda"]');

  await expect(gallery).toHaveAttribute('data-saas-killa-state', 'closed');
  await expect(itinerary).toHaveAttribute('data-saas-killa-state', 'closed');
...
test('a host page on another origin renders the gallery and the itinerary', async ({
  ...
  await expect(gallery).toHaveAttribute('data-saas-killa-state', 'ready');
  await expect(itinerary).toHaveAttribute('data-saas-killa-state', 'ready');
...
test('an unknown widget name and a malformed filter both fail soft', async ({ page, baseURL }) => {
  await openHostPage(
    page,
    baseURL!,
    '<div data-saas-killa="sponsors"></div><div data-saas-killa="agenda" data-track="banana"></div>',
  );

  const unknown = page.locator('[data-saas-killa="sponsors"]');
  await expect(unknown).toHaveAttribute('data-saas-killa-state', 'error');
  ...
  await expect(page.locator('[data-saas-killa="agenda"]')).toHaveAttribute(
    'data-saas-killa-state',
    /ready|closed/,
  );
