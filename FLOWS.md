...
| 1 `ai-evaluator@sessionboard.local` (`isBot: true`) | `reviewer` | 397-402 |
...
`ai-evaluator@sessionboard.local`):
...
row, address from `personaEmail(name)` → `<slug>@sessionboard.local`).
...
- **Steps:**
  1. The host page carries `<div data-sessionboard="agenda"></div>` and
  ...
  3. The script (`src/lib/embed-script.ts`) derives `ROOT` from `document.currentScript.src` by
     ...
     does nothing.
  4. `ensureStyle()` injects `EMBED_CSS` once under `id="sessionboard-embed-css"`.
  5. `start()` runs on `DOMContentLoaded` (or immediately if the document is already parsed) and
     mounts every `[data-sessionboard]` node.
  6. `mount(node)` returns early if `data-sessionboard-state` is already set, so a node is never
     mounted twice.
  ...
  9. The container's `data-sessionboard-state` walks `loading` → `ready` (feed published) or
...
- **Error and refusal paths:**
  - Non-2xx response → `throw new Error('HTTP ' + response.status)` → `fail(node, 'The ' + name
    ...
    `console.warn('[sessionboard] ' + detail)`.
...
- **Steps:** `mount` resolves `render` as `name === 'speakers' ? renderSpeakers : name ===
  ...
  `Unknown Saas Killa widget "posters". Use data-sessionboard="speakers" or
...
- **Steps:**
  ...
  2. `embedDocument` (`src/lib/embed.ts:341`) emits a full `<!doctype html>` with `EMBED_CSS`
     ...
     `{ sessionboardHeight: document.documentElement.scrollHeight }` to `parent` on load and on
...
- **Steps:** `GET` (`src/app/embed/demo/route.ts`) returns a hand-written HTML document, its own
  ...
  event name, an `Our speakers` heading over `<div data-sessionboard="speakers">`, a
  `The schedule` heading over `<div data-sessionboard="agenda">`, and a script tag pointing at
...
- **Steps:**
  ...
  3. The script's `renderSpeakers`/`renderAgenda` produce the same two sentences as text nodes,
     and `mount` sets `data-sessionboard-state="closed"`.
